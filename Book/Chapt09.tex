...
\begin{quotation}   Given a plant: $P(s)$,  specify a controller, $C(s)$, for a closed loop system to meet  some performance measure requirements
...
\begin{aligned}
C(s) =& {K_P + K_I/s + K_Ds}\\
=& \frac{K_Ps + K_I+K_Ds^2}{s}\\
=& \frac{K_D(s^2 + \frac{K_P}{K_D}s+ \frac{K_I}{K_D})}{s}\\
...
Assuming that we use a PID controller, we can place two zeros in the $s$-plane to get the plant's root
...
\begin{Example}\label{ExampLargeIndustrialPlant}
...
%   TBD:  This listing is out of date
%       (maybe skip!)
%
%
% \begin{listing}
...
zeros: $s= \{-1,-5.5\}$
% (Uncommenting Listing \ref{ex14Listinga} lines 30,32),
...
\end{ExampleCont}
\begin{ExampleCont}
With $K_D$ (4.6) and our PID zeros ($\{-1, -5.5\}$) known, we use
Equation \ref{PID2} and the technique in Section \ref{Kpderive}
(below) as follows:

\[
C(s) = \frac{4.6(s+1)(s+5.5)}{s} = \frac {4.6(s^2+6.5s+5.5)} {s}
\]
\[
...
\[
K_P/4.6 = 6.5, \; K_I/4.6 = 5.5
\]
\vspace{0.15in}
\[
K_P = 29.9 \qquad K_I = 25.3  \qquad K_D = 4.6
\]
\end{ExampleCont}
...
Now we place this system into a python script
and we add a little bit of code to
...
} % end of query (frmlry \end{verbatim}
\end{Example}
Giving:
...
because it has the same number of poles and zeros, two.  Thus, we can simulate it and it is also physically realizable.	%<h>
...
\humanquery{
\paragraph{Prompt 1}
 Could you please complete the following python code to perform the Root Locus analysis?
(which is followed immediately in the same prompt with the
code of Listing \ref{lstInitialAIPrompt}).} % end of query (frmlry \end{verbatim}
...
\humanquery{ Thank you.   Could you revise that to add lines to the plot which
 indicates 10\% overshoot, and settling time <= 8 sec.?
} % end of query (frmlry \end{verbatim}
...
(Note the relevant poles and zeros are kind of tiny and the
red-dashed settling time line is barely visible). Next
we ask it to fix the issues identified above:



\humanquery{\paragraph{Prompt 3}
 Thank you, The overshoot line looks good but there are spurious lines
 in the plot legend.   Also, can you change the plotting axes to
 focus on the poles and zeros  of magnitude less than 20?
} % end of query (frmlry \end{verbatim}
...
\humanquery{
\paragraph{Prompt 4}
  Since my performance specs are equality constraints, I want the diverging
poles to go near or through the intersection of the Ts and percent overshoot
lines.  Can you modify the controller for that?  Also, please further zoom to
focus on that point.
} % end of query (frmlry \end{verbatim}
...
It's very cool that Claude.ai moved the PID controller zeros to
...
\humanquery{
\paragraph{Prompt 5}
Could you add to your previous code to produce a step response for Gain = 0.512?
Also, can you add lines indicating the percent overshoot (horizontal line) and
the settling time spec (e.g. 2% window starting at the Ts spec)?
} % end of query (frmlry \end{verbatim}
...
\humanquery{ Hmm..  not so great. But we could maybe take it from here ourselves! }

% \begin{listing}
%     \begin{minted}{python}
%
% \end{minted}\caption{CAPTION TEXT}\label{*unique label*}
% \end{listing}
