...
function []=boderad(varargin)
  ...
  fname="boderad";//for error messages
  fmax=[]
  ...
  frq=frq';d=d',phi=phi'
  frq = frq*2*%pi;            // convert to rad/sec!!!
  [n,mn]=size(d)
  ...
  xtitle("",_("Frequency (rad/sec)"),_("Magnitude (dB)"));
  ...
  xtitle("",_("Frequency (rad/sec)"),_("Phase (degree)"));
  // create legend
  if comments<>[] then
